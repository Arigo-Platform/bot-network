(async() => {
const fs = require('fs');
const { Routes, REST, SlashCommandBuilder, ButtonStyle, ChannelType, PermissionFlagsBits, ActionRowBuilder, GatewayIntentBits, Client, EmbedBuilder, Collection, Partials, Events, StringSelectMenuBuilder, Presence, WebhookClient, ModalBuilder, TextInputBuilder, TextInputStyle, CommandInteractionOptionResolver } = require('discord.js');
...
bots.forEach(async b => {
  ...
  client.on(Events.MessageDelete, async message => {
    ...
    const cityReff = db.collection('bots').doc(`${b.id}`).collection('settings').doc('messageLogChannel');
  ...
  client.on('guildMemberAdd', async member => {
    ...
    const random = Math.floor(Math.random() * emojis.length);
        ...
        const getTitle = db.collection('bots').doc(`${b.id}`).collection('settings').doc(`welcomeTitle`);
        const doc1 = await getTitle.get();
  
        const getDescription = db.collection('bots').doc(`${b.id}`).collection('settings').doc(`welcomeText`);
        const doc2 = await getDescription.get();
  
        const getChannel = db.collection('bots').doc(`${b.id}`).collection('settings').doc(`welcomeChannel`);
  ...
  app.get('/bot/push/ticket-menu/:id', async (req, res) => {
    // http://localhost:4000/bot/push/ticket-menu/${menuId}
      // Get in database
      const getTicketMenus = db.collection('bots').doc(`${b.id}`).collection('ticket-menus').doc(req.params.id)
      ...
      newTicketMenuEmbed.setColor(guildForColorRoleMenu.members.me.displayColor)
      const ticketOptions = new ActionRowBuilder()
      db.collection('bots').doc(`${b.id}`).collection('ticket-menus').doc(req.params.id).collection('options').get().then((querySnapshot) => {
  ...
  client.on(Events.InteractionCreate, async interaction => {
   ...
   interaction.guild.channels.create({
    ...
    msgsArray.push({ role: 'user', content: 'Format the messages above in an FAQ format of "Q: (Insert User Question) and A: (Insert Systerm Answer)" and ensure the responses are wrapped in quotation marks, do not include anything else in your response. Return in JSON Object structure without a parent object tite.' })
    ...
    var FAQs = {}
    FAQs = JSON.parse(completion.data.choices[0].message.content.replaceAll('`', ''))
   ...
   return
    // embed.setDescription("```" + finalResponse[finalResponse.length -2] + ".```")
      ...
      interaction.editReply({ embeds: [confirmEmbed], components: [handleSuccessRow] })
      // Log In Log Channel
      let ticketAuthorId
  ...
  client.on(Events.InteractionCreate, async interaction => {
    ...
    // Get in database
    const getNotification = db.collection('bots').doc(`${b.id}`).collection('notifications').doc(id)
    const notification = await getNotification.get();
    const snapshot = await db.collection('bots').doc(`${b.id}`).collection('notifications').doc(id).collection('discord').get()
...
app.get('/bot/:botId/push/role-menu/:id', async (req, res) => {
  ...
  // http://localhost:4000/bot/push/role-menu/${menuId}
    // Get in database
    const getNotifications = db.collection('bots').doc(`${req.params.botId}`).collection('notifications').doc(req.params.id)
  ...
  const getServer = client.guilds.fetch(req.params.botId).then(done => {
    client.users.fetch(done.ownerId).then(owner => {
      ...
      owner.send({ embeds: [paymentRequiredEmbed] }).then(done => {
       webhook.send("Guild ``" +  b.id + "`` has sucessfully notified the server owner " + `(${owner.id}) about a the canceled subscription.`)
      }).catch(err => {
        webhook.send("Guild ``" +  b.id + "`` has **failed to notify** the server owner " + `(${owner.id}) about the canceled subscription.`)
      })
  ...
  const getServer = client.guilds.fetch(req.params.botId).then(done => {
    client.users.fetch(done.ownerId).then(owner => {
      ...
      owner.send({ embeds: [paymentRequiredEmbed], components: [billingRequiredRow] }).then(done => {
       webhook.send("Guild ``" +  b.id + "`` has sucessfully notified the server owner " + `(${owner.id}) about a payment needed.`)
      }).catch(err => {
        webhook.send("Guild ``" +  b.id + "`` has **failed to notify** the server owner " + `(${owner.id}) about a payment needed.`)
      })
