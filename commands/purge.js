    ...
    // See if they have the General Moderator role
     if(interaction.member.roles.cache.has(doc.data().id) === true) {
        ...
        if (amount >= 1) {
            ...
            events.info('Purge', { user: `${userId}`, amount: `${amount}`, channel: `${channel.id}`, serverId: `${serverId}` });

        // Send Success Message
        embed.setTitle("🎉 Success")
        embed.setDescription(`I've successfully purged ${amount} message(s)`)
        embed.setColor("Green")
        interaction.reply({ embeds: [embed], ephemeral: true })
       
        // Send logs to log channel
        embed.setTitle("📜 Messages Purged")
        embed.setDescription(`${username} (${userId}) has purged messages in a channel.`)
        embed.addFields(
        ...
        logChannel.send({ embeds: [embed] })
            ...
            embed.setDescription("You cannot purge 0 messages, please try with a value greater than 0.")
            embed.setColor("Red")
