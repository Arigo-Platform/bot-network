...
module.exports = {
  ...
  async execute(interaction, embed, db, events, Sentry) {
    try {
      ...
      if (
        blacklisted.some((blacklist) => text.toLowerCase().includes(blacklist))
      ) {
        (async () => {
            ...
            blocks: [
              {
                ...
                text: {
                  type: "mrkdwn",
                  text:
                    `*${interaction.user.username}* (${interaction.user.id}), has utilized the ` +
                    "`/say`" +
                    ` command in ${interaction.guild.name} (${interaction.guildId}) with potential malicious intent.`,
                },
              },
              {
                type: "section",
