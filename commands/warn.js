...
module.exports = {
  ...
  try {
    ...
    // Get Warn Moderator ID
    const cityRef = db.collection('bots').doc(`${serverId}`).collection('settings').doc('warnModId');
    ...
    // See if they have the Warn Moderator role
    ...
    if(offenderMember.moderatable === true) {
      ...
      embedtoSend.setDescription("You have been warned in ``" + interaction.member.guild.name + "``" + ` by <@${userId}> (${userId}).`)
      ...
      // Send DM to offender
        ...
        embed.setDescription(`I was unable to DM the user due to their privacy settings, although they've still been warned.`)
        embed.setColor("Red")
        interaction.followUp({ embeds: [embed], ephemeral: true })
      }, 500);
    });   
    
        events.info('Warn', { user: `${userId}`, offender: `${offender.id}`, reason: `${reason}`, serverId: `${serverId}` });
        embed.setFields([]);;
        // Log in database
        ...
        embed.setTitle("📜 Server Warn Issued")
        embed.setDescription(`<@${offender.id}> (${offender.id}) has been warned.`)
        embed.addFields(
        { name: 'Reason', value: `${reason}`, inline: true },
        { name: 'Case ID', value: `${caseId}`, inline: true },
        ...
        logChannel.send({ embeds: [embed] })
          ...
          // The bot cannot warn the user
          embed.setTitle("😞 Insufficient Permissions")
          embed.setDescription("I was unable to warn this member, please ensure my role is higher than the offender and be sure you're not trying to warn the server owner.")
          embed.setColor("Red")
