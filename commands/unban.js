        ...
        if (interaction.member.roles.cache.has(doc.data().id) === true) {
          ...
          try {
            ...
            embed.setDescription(
              "This user isn't banned from the server so therefore no action has been taken. Please ensure the provided information is correct and rerun the command."
            );
            embed.setColor("Red");
          ...
          embed.setDescription(
            `The server unban has been issued to <@${offender.id}> (${offender.id}) and has been successfully logged.`
          );
          ...
          // Unban the user
          interaction.guild.bans.remove(offender.id, {
            reason: `This user was unbanned by ${username} (${userId}) for "${reason}"`,
          });
          events.info("Unban", {
            user: `${userId}`,
          ...
          embed.setDescription(
            `<@${offender.id}> (${offender.id}) has been unbanned.`
          );
          embed.addFields(
            { name: "Reason", value: `${reason}`, inline: true },
            { name: "Case ID", value: `${caseId}`, inline: true },
