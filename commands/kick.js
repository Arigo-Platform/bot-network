        ...
        if (interaction.member.roles.cache.has(doc.data().id) === true) {
          ...
          if (offenderMember.kickable === true) {
            ...
            embed.setDescription(
              `The server kick has been issued to <@${offender.id}> (${offender.id}) and has been successfully logged.`
            );
            ...
            offender.send({ embeds: [embedtoSend] }).catch((error) => {
              setTimeout(() => {
                ...
                embed.setDescription(
                  `I was unable to DM the user due to their privacy settings, although they've still been kicked.`
                );
                embed.setColor("Red");
            ...
            offenderInfo.kick(
              `This user was kicked by ${username} (${userId}) for "${reason}"`
            );
            events.info("Kick", {
              user: `${userId}`,
            ...
            embed.setDescription(
              `<@${offender.id}> (${offender.id}) has been kicked.`
            );
            embed.addFields(
              { name: "Reason", value: `${reason}`, inline: true },
              { name: "Case ID", value: `${caseId}`, inline: true },
            ...
            embed.setDescription(
              "I was unable to kick this member, please ensure my role is higher than the offender and be sure you're not trying to kick the server owner."
            );
            embed.setColor("Red");
