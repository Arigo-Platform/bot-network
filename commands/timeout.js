...
module.exports = {
  ...
  try {
    ...
    if(offenderMember.moderatable === true) {
      ...
      embedtoSend.setDescription("You have been timed out in ``" + interaction.member.guild.name + "``" + ` by <@${userId}> (${userId}).`)
      ...
      embed.setDescription(`I was unable to DM the user due to their privacy settings, although they've still been timed out.`)
      embed.setColor("Red")
      interaction.followUp({ embeds: [embed], ephemeral: true })
        ...
        offenderMember.timeout(forkickduration * 60 * 1000, `This user was timed out by ${username} (${userId}) for ${reason}, through the Arigo Platform Bot Network. To report abuse, please contact abuse@arigoapp.com`)
        events.info('Timeout', { user: `${userId}`, offender: `${offender.id}`, reason: `${reason}`, serverId: `${serverId}` });
        ...
        embed.setDescription(`<@${offender.id}> (${offender.id}) has been timed out.`)
        embed.addFields(
        { name: 'Reason', value: `${reason}`, inline: true },
        { name: 'Case ID', value: `${caseId}`, inline: true },
        { name: 'Duration', value: `${duration}`, inline: true},
        ...
        logChannel.send({ embeds: [embed] })
          ...
          embed.setDescription("I was unable to timeout this member, please ensure my role is higher than the offender and be sure you're not trying to timeout the server owner.")
          embed.setColor("Red")
