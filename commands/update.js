...
module.exports = {
    ...
    // Change Nickname
      embed.addFields(
        { name: '✏️ Nickname Update', value: `Before → ${interaction.member.nickname}\nAfter → ${rblxUsername}`, inline: true  }
        )
    }
    // Get rank in group
    const rankId = await noblox.getRankInGroup(groupId, doc.data().rblxId)
    // See if they're applicable to a bind
    const cityRfeff = db.collection('bots').doc(`${serverId}`).collection('verification').doc(`${rankId}`);
